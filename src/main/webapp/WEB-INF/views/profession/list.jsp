...
<div id="mainPanel" class="main-panel">
	<section class="toolbar">
		<button type="button">新增职业</button>
	</section>
	<section>
		<table class="table">
			<thead>
			...
			<tbody>
				<c:forEach items="${pageBean.list}" var="profession"
					varStatus="status">

					<tr data-id="${profession.id }">
						<td>${status.count }</td>
						<td>${profession.name }</td>
						<td>${profession.creatTime }</td>
						<td><a href="${ctx}/profession/edit"
							data-id="${profession.id }">编辑</a> <a
							href="${ctx}/profession/delete" data-id="${profession.id }">删除</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
...
<script type="text/javascript">
	$(function() {
		...
		$mainPanel.find(".toolbar").find("button").on("click", function() {
			loadMainContent("/profession/add");
		});
